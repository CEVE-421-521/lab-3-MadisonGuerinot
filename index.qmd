...
*What years does the data cover?* 2000 to 2100
...
```{julia}
intermediate = noaa_slr[noaa_scenario =3]
```
...
```{julia}
median(rcp85_slr; dims = :ensemble)
```
...
rcp85_p95 = mapslices(x -> quantile(x, 0.95), rcp85_slr; dims = :ensemble)
rcp85_p95 = dropdims(rcp85_p95; dims = :ensemble)
...
The NOAA Intermediate scenario and the BRICK RCP 8.5 projections are in agreement until around 2035, and then they diverge with the NOAA projections being higher than the median BRICK proection. 
:::
...
There are many factors that will determine sea level rise that we do not currently know or understand. For example, how much the sea rises depends on how much we continue to emit. There isn't a probability attached to each scenario because this is not a probabilistic assessment, but a "what if" analysis. 
:::
...
There is less density across the 2100 distribution because these projections are more uncertain and contain higher variability. 
:::
...
The NOAA High scenario is much higher than the BRICK 90% range. 
2. *What does this tell us about the relationship between these two approaches?*
When we run an ensemble, we can estimate the variability in our results. The NOAA result may be an anomaly. 
3. *The BRICK ensemble spread represents uncertainty in model parameters. What kinds of uncertainties might this capture?*
We have uncertainties in the feedbacks between different climate factors including albedo. 
...
These are not ensemble runs, but single estimates based on different parameters. We cannot assign probabilities because we have not assessed the sensitity and uncertainty in the paramaters. 
...
The scenario is a single example of what might happen in a given set of circumstances. The projection represent the range of probabilistic outcomes of a given scenario. 
- *Why do climate scientists use scenarios instead of making single predictions about the future?*
Climate scientists like projections better so that we can quantify the uncertainty of our estimates. This let's us say with confidence that the range of sea level rise is between two values.
...
1. To design the seawall, I would use the worst case scenario from NOAA. If the design must last until 20250, then we need to design it for the worst possible scenario. 
2. For the zoning restrictions, I would use the projections. These restrictions are set to last for a very long time. I want to know more about the uncertainty so that I can balance safety with economic interests.
:::
...
While we do not know the exact number, we have good estimates on the range of the rise and we know that it will rise at least some. Therefore we can use the projections to decide our risk tolerance and balance it with the money spent. 
:::
